...
def send_checklist_email(instance, transformers_data, current_transformers_data):
    ...
    subject = f"Checklist Subestações PMT - {title}"
    ...
    email = EmailMessage(
        subject=subject,
        body=f"Checklist subestações PMT ({title}) foi respondido.",
    ...
    with open(pdf_file, "rb") as pdf_file:
        email.attach(f"Checklist F {title}.pdf", pdf_file.read(), "application/pdf")
